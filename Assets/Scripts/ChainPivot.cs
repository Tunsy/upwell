...
public class ChainPivot : MonoBehaviour {
	...
	void Start () {
        ...
        float pivotRad = transform.localScale.y/2;
		for(double i = 0; i < 360; i += 360/ segments)
        {
            GameObject segment = GameObject.Instantiate(seg);
            segment.transform.localScale = new Vector3(seg.transform.localScale.x, diameter, 0);
            ...
            segment.transform.position = new Vector2(transform.position.x, transform.position.y - (pivotRad + diameter/2) );
